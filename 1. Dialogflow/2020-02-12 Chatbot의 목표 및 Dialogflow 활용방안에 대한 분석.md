# 1. Chatbot의 목표
> * 1단계: 정해진 질문에 대한 응답만 제출할 수 있는 챗봇을 구현
> * 2단계: 사용자가 챗봇에게 코드와 같은 질문을 하였을 경우, 해당 질문과 가장 유사한 질문을 Q&A게시판(향후 구현)에서 찾아 추천(2단계에서는 질문에 대한 응답이 Q&A시스템에 존재할 경우 URL을 하이퍼링크로 사용자에게 제공해준다.)해주거나, 이미 챗봇이 알고 있는 내용일 경우 챗봇이 응답할 수 있도록 구현
> * 3단계: Q&A시스템에 응답을 특정한 검증과정을 거쳐 챗봇이 학습하여 다음에 비슷한 질문을 사용자가 챗봇에게 했을 때, 학습한 응답을 사용자에게 전송 
> * 4단계: 챗봇이 모든 데이터를 학습하여 챗봇자체에서 단어를 조합해 새로운 문장을 제시하거나, 코드를 작성할 수 있도록 개발
> 
> * 다음과 같은 4가지의 과정을 순차적으로 해결하기 위하여, Dialogflow를 사용해 분석을 하고, 적합성을 판단해 보았다.

### 1.1 Dialogflow란 무엇인가?
 * 머신러닝엔진(Natural Language Understanding)을 사용해 자연어를 처리/분석 챗봇 플랫폼이다.
 * 코딩을 지양하고, 대화의 흐름만으로 챗봇을 만들 수 있도록 UI를 설계를 할 수 있다.
 * 예약과 주문과 같은 판매직종이나, 예약이 필요한 모든 직종에 사용하고 있다.
 ...
 * 대화순서: 아티스트 입력(사용자) -> 해당 아티스트와 관련된 이벤트 정보를 사용자에게 전달(챗봇) -> 이벤트정보를 확인하고 좌석을 선택(사용자) -> 해당 좌석의 가격을 사용자에게 알려줌 -> 사용자는 가격에 대한 정보를 받고 결제를 진행(사용자) -> 결제된 내용을 바탕으로 결제내역과 예약정보를 사용자에게 전송(챗봇)
 ...
 * 챗봇의 응답을 받았을 때, 엔터키가 지원되지 않는다는점 -> 가독성이 매우 떨어진다. 
 * 이를 해결하기 위해서는 챗봇과 연동되는 특정 어플리케이션을 이용해야한다.(페이스북 메신저, kik, Slack, Skype, Telegram integration) 이외의 플랫폼에서는 엔터키가 지원되지 않아 사용자들이 보기에 매우 불편하다. -> 직접 브라우저를 구성하거나 어플리케이션을 만들 경우 적합하지않음.
...
# 4. 이외의 Chatbot플랫폼 또는 Chatbot유형 (다음 중 한가지를 선택하여 분석 및 구현을 진행해 볼 예정)
